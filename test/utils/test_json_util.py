...
def test_ensure_defaults(tmp_path, ju):
    cfg_dir = tmp_path / "cfg"
    defaults_file = cfg_dir / "defaults_ui.json"
    default_data = {"font_size": 18}

    ju.ensure_defaults(cfg_dir, defaults_file, default_data)
...
def test_load_json(tmp_path, ju):
    file_path = tmp_path / "test.json"
    test_data = {"a": 1}
    with open(file_path, "w") as f:
        json.dump(test_data, f)

    loaded_data = ju.load_json(file_path)
    assert loaded_data == test_data


def test_load_json_non_existent(tmp_path, ju):
    file_path = tmp_path / "non_existent.json"
    loaded_data = ju.load_json(file_path)
    assert loaded_data == {}


def test_save_json(tmp_path, ju):
    file_path = tmp_path / "subdir" / "test.json"
    test_data = {"b": 2}

    ju.save_json(file_path, test_data)
...
def test_backup_file(tmp_path, ju):
    file_path = tmp_path / "settings_ui.json"
    file_path.write_text("content")

    ju.backup_file(file_path, max_backups=2)
...
def test_backup_file_rotation(tmp_path, ju):
    file_path = tmp_path / "settings_ui.json"
    file_path.write_text("content")
