...
class TestFileList:
    ...
    def test_ui_initialization(self, file_list):
        ...
        assert file_list.file_view.count() == 0
    ...
    def test_update_file_list(self, file_list, mock_file_items):
        ...
        assert file_list.file_view.count() == 3
        assert "video1.mp4" in file_list.file_view.item(0).text()
        assert "video2.mkv" in file_list.file_view.item(1).text()
        assert "readme.txt" in file_list.file_view.item(2).text()
        assert (
            file_list.file_view.item(0).data(Qt.ItemDataRole.UserRole)
            == "/path/to/video1.mp4"
        )
    ...
    def test_set_selected_file(self, file_list, mock_file_items):
        ...
        assert file_list.file_view.currentRow() == 1
        assert (
            file_list.file_view.currentItem().data(Qt.ItemDataRole.UserRole)
            == target_path
        )
    ...
    def test_emit_file_selected(self, file_list, mock_file_items, qtbot):
        ...
        item = file_list.file_view.item(0)
    ...
    def test_apply_theme(self, file_list):
        ...
        with (
            ...
            assert "list { color: yellow; }" in file_list.file_view.styleSheet()
            assert file_list.file_view.font().family() == "Arial"
            assert file_list.file_view.font().pointSize() == 14
